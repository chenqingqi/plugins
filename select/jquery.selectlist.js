    ...
    SelectList.prototype={
        ...
        setSelectEvent: function(element){
            ...
            if(that.settings.enable){
                ...
                {
                	$(selectID).find('input[type="hidden"]').val(v)
                	
              		var options = $(selectID).find('.select-list li');
                	options.removeAttr('class');
              		$(options[v]).attr('class','selected');
             		$(selectID).find('.select-button').val($(options[v]).text());
                }
                ...
                $(selectID + ' li').on('click',function(event)
                {
                     ...
                     * 列表焦点已改变事件
                     */
                    $(selectID).trigger('change',{value:$(this).attr('data-value'),text:$(this).text()});
